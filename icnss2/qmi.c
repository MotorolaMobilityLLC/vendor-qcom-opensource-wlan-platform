...
static int get_moto_device(void)
{
	...
	if (rc || !bootdevice){
		...
		strscpy(device_ptr, bootdevice,MOTO_STRING_LEN);
		return 0;
	}
}
...
static int get_moto_sku(void)
{
	...
	if (rc || !sku_val) {
		...
		strscpy(sku_ptr, sku_val, MOTO_STRING_LEN);
		return 0;
	}
}
...
static int find_radio_by_sku(char* sku_val)
{
	...
	while(moto_sku_radio_map_list[i].sku != NULL) {
		if (!strncmp(moto_sku_radio_map_list[i].sku, sku_val, strlen(sku_val))) {
			strscpy(radio_ptr, moto_sku_radio_map_list[i].hw_radio, sizeof(radio_ptr) - 1);
			return 0;
		} else {
			i++;
			continue;
